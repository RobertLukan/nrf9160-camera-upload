...
extern struct cameraControl cameraCtr;
...
//		LOG_INF("%s bytes received from cloud",grx_buf);
		...
		const cJSON *msec = NULL;
		const cJSON *qValue = NULL;
		const cJSON *cameraFreq = NULL;
    	...
    	if (cJSON_IsString(cGpio) && (cGpio->valuestring != NULL))
    	{
			...
			else if(!strcmp(cGpio->valuestring,"GPIO"))
			{
				LOG_INF("Mathched \"%s\"\n", cGpio->valuestring);
				cameraCtr.takePictureNow = true;
			}
			else if(!strcmp(cGpio->valuestring,"DELAY"))
			{
				LOG_INF("Mathched \"%s\"\n", cGpio->valuestring);
				msec = cJSON_GetObjectItemCaseSensitive(json, "msec");
				cameraCtr.delayUdpSend = msec->valueint;
			}
			else if(!strcmp(cGpio->valuestring,"QUALITY"))
			{
				LOG_INF("Mathched \"%s\"\n", cGpio->valuestring);
				qValue = cJSON_GetObjectItemCaseSensitive(json, "value");
				cameraCtr.quality = qValue->valueint;
			}
			else if(!strcmp(cGpio->valuestring,"CAMERAFREQ"))
			{
				LOG_INF("Mathched \"%s\"\n", cGpio->valuestring);
				qValue = cJSON_GetObjectItemCaseSensitive(json, "value");
				cameraCtr.cameraFrequency = qValue->valueint;
			}
			else if(!strcmp(cGpio->valuestring,"SOLARCHARGERENABLE"))
			{
				LOG_INF("Mathched \"%s\"\n", cGpio->valuestring);
				qValue = cJSON_GetObjectItemCaseSensitive(json, "value");
				cameraCtr.solarChargerEnable = qValue->valueint;
			}
			else
			{
	...
	//	char gpio = "gpio";
	//	memcpy(destination,nrf_cloud_evt->data.ptr, nrf_cloud_evt->data.len);

	//	if(decode_cloud_message(&nrf_cloud_evt, "temp", "read")) {
...
//		}
	...
	case NRF_CLOUD_EVT_FOTA_ERROR:
		...
		break;
	default:
		LOG_INF("Unknown event type: %d", nrf_cloud_evt->type);
		break;
	}
}
